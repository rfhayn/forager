...
Source: `investigate-import-and-store-test-failures` (2026-04-19). Prior behavior (M9.23 commit `e058ef7`) broadened the refresh from cross-store-only to all-updated-objects to fix 134040 validation errors; that broadening silently discarded `replaceExistingRecipe`'s target-recipe edits. The `preserveUpdated` escape hatch preserves M9.23's fix for cross-store cases while letting replace-recipe edits flow through.
...
- **THEN** after save, the recipe's title matches the draft's title
...
- **THEN** after save, the recipe's `ingredients` relationship contains exactly the new draft ingredients

#### Scenario: Save-import remains unchanged

- **WHEN** `RecipeImportService.saveImport(from:)` is invoked
- **THEN** `persistAndFinish` is called with the default empty `preserveUpdated` set AND the cross-store-safety refresh loop still runs on all pre-existing updated objects (preserving the M9.23 fix for 134040 validation errors)
