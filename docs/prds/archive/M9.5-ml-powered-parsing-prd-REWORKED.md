...
**Milestone**: M9.5 → **M8.4** (Ingredient Parsing Intelligence - Phase 4, OPTIONAL)
**Status**: ⏳ PLANNED (OPTIONAL)
**Version**: 1.1
**Created**: December 19, 2025
**Updated**: January 4, 2026 (Renumbered as M8.4 in new milestone structure)
**Dependencies**: M8.3 complete (substantial training data from user corrections)

> **NOTE**: This PRD's content is now **M8.4: ML-Powered Parsing (OPTIONAL)** in the new milestone structure. See [M8 Meta-PRD](../m8-ingredient-parsing-intelligence-meta-prd.md) for the complete 4-phase parsing intelligence roadmap (M8.1-M8.4) and decision framework for whether to pursue this optional phase.
...
**Problem**: Even hybrid NLP (M8.3) has limits
...
**Returns**:
- Accuracy: 98% → 99.5% (+1.5% absolute)
- Edit rate: 2% → 0.5% (-75% relative)
- Portfolio value: Showcases ML expertise
- Competitive edge: Best-in-class parsing

**Trade-off**: Diminishing returns—98% is already professional-grade
...
- ✅ Custom CoreML model trained on real user data
- ✅ On-device inference (privacy-first)
- ✅ 99.5%+ parsing accuracy on YOUR users' inputs
...
**Dependencies**: M8 complete, ≥100 user corrections collected
...
- [ ] Have ≥100 user corrections in telemetry
...
**Dependencies**: M8 complete with ≥100 user corrections  
