...
- `harden-factory-enforcement-for-child-entities` (~12–16h) — migrate the ~18 `GroceryListItem(context:)` / `Ingredient(context:)` production sites to `ManagedObjectFactory.make()`. Removes the child-inheritance pattern that ADR 014 currently sanctions but that caused CloudKit zone conflict error 134040 in April 2026 (see archived change `fix-groceryitem-multi-zone-assignment`). Requires factory injection into services (GroceryListItemService, RecipeService, MealPlanService, WeeklyListService, QuantityMergeService, IngredientParsingService) and SwiftUI `@Environment(\.managedObjectFactory)` wiring into views (WeeklyListsView, AddIngredientsToListView, EditRecipeView, RecipeListView, CreateRecipeView). Matches the `DataScope.swift:98-103` protocol doc's strict interpretation that factory is the "ONLY creation path." The `fix-groceryitem-multi-zone-assignment` change (April 2026) takes the interim path: keep child-init + mandate explicit `context.assign()`, enforced by the architecture-guard hook.
