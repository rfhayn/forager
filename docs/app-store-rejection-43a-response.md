...
| 2026-07-24 | **Old app record DELETED; fresh app record created** | Developer decision after the round-4 rejection: the poisoned context was the submission/record itself (4 rejections + Board uphold, all on `e5e960e5…`), so the developer canceled the submission and deleted the app record entirely, then a fresh record was built the same night: **"forager - Groceries, managed."** (App ID **6794530669**, new bundle ID `com.richhayn.foragerapp` — old bundle ID is permanently unusable after deletion). CloudKit container `iCloud.com.richhayn.forager` reattached to the new App ID (hardcoded in entitlements, so all production household data survives — verified on device via TestFlight build 156). v1.0 (156) cut from the same post-reskin code; metadata rebuilt from the 2026-04-23 repositioning copy (recovered from unmerged commit `7276217`); all 10 Provisions Press screenshots re-uploaded and both sets enumerated; privacy labels published (Data Not Collected); full 2026 age-rating questionnaire completed (17+ via Unrestricted Web Access). Review notes rewritten **cold** — no history, no ticket references. |
| 2026-07-25 | **FRESH SUBMISSION SUBMITTED** | Submission **`82a664a2-4ff1-4196-923c-ff40e61325f6`** (v1.0, build 156) submitted 03:32 UTC (evening 2026-07-24 ET), state WAITING_FOR_REVIEW. First submission in the saga with zero inherited reviewer context: new record, new submission ID, first-release version string, cold notes. EU distribution deferred (trader status undeclared — account-level, non-blocking). On verdict: log here; if rejected again under 4.3(a), escalation path is Meet with Apple #2 with the compliance paper trail, then the executive-escalation letter (see session-133 strategy analysis). |
