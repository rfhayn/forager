...
**Implication**: Phase 1 must include a `WKWebView` fallback path that renders the page with JavaScript before extracting JSON-LD. This raises the extraction rate from ~43% to an estimated ~75-80%.
...
| Full extraction rate (title + ingredients + instructions) | ≥ 55% | Spike: 43% without WKWebView; target with WKWebView is 55%+ |
...
|-------|----------------------|--------------|-------|
| Title | 100% | 100% (12/12) | Always present in Recipe JSON-LD |
| Ingredients | 100% | 100% (12/12) | recipeIngredient is the core field |
| Instructions | ≥ 95% | 100% (12/12) | Always present when Recipe @type is found |
| Prep time | ≥ 75% | 83% (10/12) | ISO 8601 parsing; missing on paywall sites |
| Cook time | ≥ 70% | 75% (9/12) | Missing on Serious Eats + paywall sites |
| Servings | ≥ 90% | 100% (12/12) | Various formats handled by yield parser |
| Image URL | ≥ 95% | 100% (12/12) | Always present |
| Author | ≥ 80% | 83% (10/12) | Missing on Budget Bytes, RecipeTin Eats |
