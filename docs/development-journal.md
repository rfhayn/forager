...
## Session 133 — July 24, 2026 — Round-4 rejection → fresh app record ("forager - Groceries, managed.")

**Change**: `refile-fresh-app-record` (new) — the post-reskin resubmission was REJECTED (fourth 4.3(a), 2026-07-14, build 155, iPhone 17 Pro Max; boilerplate reply again 07-15), and the response is structural: delete the poisoned app record entirely and refile as a brand-new app.

**What happened**: The Provisions Press reskin did not move the template — same 4.3(a) wording, same "The issues we previously identified still need your attention" opener, zero engagement with the point-by-point consultation-compliance reply. That disproves the design-surface hypothesis *as sufficient* (and the July 2 rep's "solvable in place" guidance with it). The session's diagnosis: after four rejections + a Board uphold, all on ONE submission ID, the thread itself was the trap — a triage reviewer inheriting a Board-upheld verdict will not overturn it, regardless of what changed in the app. Fresh research added a second, external factor: **Apple hardened guideline 4.3 on June 8, 2026** ("Don't submit apps that are indistinguishable from what's already widely available"; oversaturated categories need a "meaningfully different or improved experience") — the July 14 review ran under a stricter regime than the consultation was calibrated to. Every remaining lever changes *who reviews it and what they think they're classifying*, not the app.

**Key decisions** (Rich's calls, made fast): keep the name family but sharpen it — **"forager - Groceries, managed."** (29 chars, deadpan editorial voice matching Provisions Press; subtitle keeps carrying the differentiators) — refile immediately rather than book Meet with Apple #2 first, and go all the way to a **new app record** rather than just a fresh submission on the old one. Rich canceled the submission and deleted the record himself; the permanent costs were priced in beforehand: bundle ID `com.richhayn.forager` is burned forever (builds were uploaded), SKU unusable, TestFlight history + tester list wiped.

**Execution**: New bundle ID `com.richhayn.foragerapp` (registered via ASC API, `CJDZB23DMA`) with ICLOUD (XCODE_6), PUSH_NOTIFICATIONS, and ASSOCIATED_DOMAINS capabilities added by API. The saving architectural grace: the CloudKit container `iCloud.com.richhayn.forager` is hardcoded in entitlements + 3 Swift call sites rather than derived from the bundle ID — containers are account-level and bundle-decoupled, so the new app opens the same production container and every household/recipe/list survives. Project changes: bundle ID swap, `MARKETING_VERSION` 2 → 1.0 (a fresh record reads best as a first submission), stale `healthcare-fitness` LSApplicationCategoryType → `food-and-drink`, Info.plist URL-type name updated, camera-permission copy lowercased per the copy rule. App record created by driving Chrome (new App ID **6794530669**); new beta groups minted via API — an *internal* group with `hasAccessToAllBuilds` (instant availability, no beta review — Rich smoke-tests tonight) plus a recreated external "Public Beta Testers" (`408cdc6a…`); `testflight-distribute.sh` repointed. First archive attempt failed on provisioning (new App ID had no profile with the capabilities); re-ran with `-allowProvisioningUpdates` + API-key auth so cloud signing syncs the App ID and attaches the container.

**Learning**: (1) An ASC "submission" is not just a queue ticket — it's the unit of reviewer context. Four rounds of evidence: nothing submitted on a thread carrying an upheld appeal gets evaluated on its merits. The §11.6 withdraw-and-refile instinct from June was right; deferring it twice cost two rounds. (2) Deleting an app record has three permanent, poorly-advertised costs (bundle ID, SKU, name ownership) and one big survivor (CloudKit containers) — hardcoding container IDs instead of deriving from bundle ID is what made the restart cheap. (3) ASC's New App dialog rejects programmatic `<select>` mutation — native type-ahead (focus, type the option label, Enter) is the reliable automation path. (4) June 8, 2026 guidelines update is required reading for any future submission argument: the bar is now "meaningfully different or improved experience," which is a *classification* test — fought in the name/subtitle/first-screenshot, not in the binary.

**AI tooling observations**: The strategy phase ran research-first (web search surfaced the June guideline change and the bundle-ID-burn rule before any irreversible step), and the execution phase interleaved three surfaces in one session — local repo edits, ASC REST API (bundle IDs, capabilities, beta groups, testers), and Chrome-driven UI for the one thing the public API can't do (app record creation). Rich executed the destructive ASC steps himself; everything reversible was automated.

**Addendum (same night — SUBMITTED)**: The entire fresh-record pipeline completed in one evening. Signing detour: cloud signing via the API key failed until the iCloud container was attached to the new App ID in the developer portal (no public API for container assignment — driven via Chrome); after that, `-allowProvisioningUpdates` + API-key auth minted the profile and the archive succeeded. Build 156 uploaded, processed VALID in ~3 minutes; Rich's internal-tester invite needed a resend (invites minted before any build exists dead-end — new insight pattern) and his device smoke test confirmed the critical property: **the existing household appeared and synced in the new-bundle app** (container decoupling verified in production). Metadata staged via API: repositioned description/keywords (recovered from unmerged commit `7276217` — the strong "Most grocery apps want you to create an account. forager doesn't." copy), subtitle, categories, Free pricing (appPriceSchedules with `${price1}` temp-ID include), full 2026 age-rating questionnaire (21 attributes now required in one PATCH, including new ageAssurance/messagingAndChat/userGeneratedContent questions), copyright (the last submit blocker), review contact + cold notes; privacy labels ("Data Not Collected") published via Chrome since they have no public API; all 10 screenshots chunk-uploaded and both sets enumerated (5+5). Submission `82a664a2…` created via API, version attached (READY_FOR_REVIEW), and Rich pressed Submit: **WAITING_FOR_REVIEW at 2026-07-25 03:32 UTC**. Post-submit verification swept every surface green. Deferred: EU trader status (non-blocking; EU excluded until declared), external beta tester re-invites (need emails), Desktop archive artifacts left in place.
...
**Addendum 8 (2026-07-07 — RESUBMITTED)**: PR #157 squash-merged; build 155 cut from main, distributed, and swapped onto the existing submission via the ASC API along with the review notes and all ten screenshots — including a surprise: the listing carried an iPad set of ORIGINAL pre-reposition captures that would have sunk the redesign story on the reviewer's own device class; replaced with iPad-size Provisions Press composites. What's New turned out not to exist (first release). The API submit hit Apple's asset-ingest lag ("version not ready"); the user took the final step in the browser with a Resolution Center reply enumerating the four design resolutions against the July 2 consultation. Also: a `git add -A` briefly committed the unredacted drafts folder — caught, amended out, force-pushed, and gitignored (the commit convention's "stage specific files" rule exists for exactly this). forager is back in the review queue for the first time since 2026-05-05.

**Addendum 7 (2026-07-07 — close-out: verified, documented, merge-ready)**: Build 154 verified on-device (Texas Chili AI parse succeeds, Diagnostics gone from Settings) — closing 5.2/5.3 for good. The AI-parse failure was diagnosed by a background agent while the main session kept shipping: `max_tokens: 1024` truncation on the all-N-ingredients-in-one-request batch path, unobservable because `stop_reason` was never checked and a three-way guard conflated every failure into "Missing tool_use." Fix: per-item output budget, explicit truncation error, split guards. Docs closed out in the same stretch: 6.2 (Meet-with-Apple outcome + §12 fresh-submission package in the 43a doc), 6.1 (style-contract/token-map promoted to docs/design-system/ with the day's rules as an addendum; CLAUDE.md + memory now teach Provisions Press), landing page final (5 screenshots, user-approved), screenshots composited (Provisions Press band + mounted-print framing after the user caught both the SF-Rounded caption band and the boundary-less blend). Diagnostics re-gated per the compliance-sweep TODO. **Strategy revision from the top**: no withdraw-and-refile — Apple said the existing submission is solvable, so we resubmit in place (§12 revised). Harness deleted per its own header; branch is merge-ready at 22/23 tasks.

**Addendum 6 (broadsheet masthead — the title system, finally singular)**: The header work went through three prototypes in one evening — 17pt inline (too small), 20pt inline (truncated "Manage Categori…" against trailing capsules), and Rich's blunt verdict on the growing zoo: "these are all still different lol." He also probed moving titles to the bottom tab bar; talked through the geometry honestly (the capsule row is mandatory, so inline titles ride free — bottom placement reclaims nothing) and it sharpened the real requirement: ONE rule. Landed on the **broadsheet masthead**: the bar is chrome (capsules only, never a title); every page names itself in print — left-aligned `detailTitle` (28 heavy condensed) tight under the capsule row, via shared `BroadsheetMasthead`. Applied to all four tab roots (replacing system large titles and their air gap), nine pushed screens, and both editable detail titles (list/plan names now render full-size with in-place long-press rename — no more bar truncation). Copy tightened to crate-label terseness: "Manage Categories" → "Categories", "Manage Stores" → "Stores", "Help & Guide" → "Help". Modal sheets keep inline bar titles (20pt condensed heavy via the UINavigationBarAppearance fix) as the one sanctioned exception. Lesson: prototype variants answered every question ("too small," "truncates") faster than argument; and the masthead only emerged after the centered-inline dead end proved titles can't share the bar with trailing actions.

**Addendum 5 (header unification)**: Rich's 10-screenshot header review exposed an iOS 26 quirk: the legacy `UINavigationBar.appearance()` proxy honors `largeTitleTextAttributes` but silently ignores `titleTextAttributes` on the new glass bars — so every large title was condensed-heavy while every INLINE title fell back to plain SF semibold ("Manage Stores" vs "Settings"). Fix: route the same fonts through a `UINavigationBarAppearance` configured with `configureWithTransparentBackground()` (sets no background → Liquid Glass material survives, verified by screenshot). Also: the two custom editable principal titles now match the global inline voice; Household's "Members"/"Shared Data" grey text headers became `ForagerBand` ink bands; and 35 raw `.headline`/`.title2`/`.title3` Text headings (sheet titles, form-block heads, empty states) converted to theme tokens — classified programmatically from 74 total sites, with the 35 Image icon-sizing uses and 4 numeric form values left alone.

**Addendum 4 (voice pass round 2 — user overruled, correctly)**: Rich pushed back on Addendum 3's direction: he *liked* the condensed face and wanted MORE of it — ingredient names in the crate-label voice, not quieter body text. Then the sharper question: does the amount even need a different font? Settled it with a two-variant simulator bake-off opened side-by-side in Preview: (C) mono amounts + condensed names vs (A) the whole line condensed. **A won — ingredient rows are now uniformly crate-label** (semibold qty+name, regular grey qualifier); mono persists only in chrome-ish counters (band counts, progress %, dates) pending a future call. The metaFont token from Addendum 3 lived for one build — replaced by `bodyCondensed`. Also shipped: display-layer decimal→kitchen-fraction conversion (`displayingKitchenFractions`, mirroring RecipeScalingService's table) — "0.25 cup" renders "1/4 cup" everywhere ingredient text shows. Lesson: on aesthetic direction, render the options and let the owner pick from pixels; my typographic reasoning (two-voice separation) was internally consistent and still not what he wanted.
